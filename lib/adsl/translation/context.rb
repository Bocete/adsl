  ...
  module Translation
    class ContextCommon
      ...
      def make_ps(prefix = 'p')
        return [] if @context_sort.nil?
        return [ADSL::Translation::TypedString.new(@context_sort, "#{prefix}1")] if @parent.nil?
        return @parent.make_ps(prefix) << ADSL::Translation::TypedString.new(@context_sort, "#{prefix}#{@level}")
      end
      ...
      def before(c2, c1var, c2var, executed_before)
        ...
        @translation.reserve(@parent.make_ps "parent_a") do |context1_names|
          @translation.reserve(c2.parent.make_ps "parent_b") do |context2_names|
            ...
            before_options = FOL::Or.new(
              (1..common_context.level).map{ |index|
                FOL::And.new(
                  (index <= 1 ? true : FOL::Equal.new(context1_names[index-2], context2_names[index-2])),
              ...
              FOL::And.new(
                FOL::Equal.new(context1_names[common_context.level-1], context2_names[common_context.level-1]),
                executed_before
              )
            )
            return FOL::ForAll.new(context1_names[0..-2], context2_names[0..-2], FOL::Implies.new(
              FOL::And.new(
