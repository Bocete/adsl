      ...
      module CallbackChainSimulator
        ...
        def halting_status_of(ast_node)
          if ast_node.is_a?(ASTBlock)
            ...
            sub_statuses = ast_node.blocks.map{ |block| halting_status_of(block) }
        ...
        def split_into_paths_that_will_or_will_not_halt(block)
          case halting_status_of block
          ...
          block.statements.each do |stmt|
            if stmt.is_a?(ASTBlock) && halting_status_of(stmt).nil?
            ...
            elsif stmt.is_a?(ASTEither) && halting_status_of(stmt).nil?
              rendering_paths     = []
              not_rendering_paths = []
              
              stmt.blocks.each do |subblock|
                possibilities = split_into_paths_that_will_or_will_not_halt subblock
